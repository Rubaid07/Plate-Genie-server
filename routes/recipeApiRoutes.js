...
module.exports = (recipesCollection) => {
  ...
  router.post('/', async (req, res) => {
    ...
    if (!userId || !title || !description) {
      return res.status(400).json({ message: 'Missing required recipe fields: userId, title, descriptio-n.' });
    }
  ...
  router.post('/:recipeId/save-toggle', async (req, res) => {
    const { recipeId } = req.params;
    const { userId } = req.body;
    ...
    try {
      ...
      res.status(500).json({ message: 'Server error while toggling save status.' });
    }
  });
  ...
  router.get('/saved/:userId', async (req, res) => {
    ...
    try {
      const savedRecipes = await recipesCollection.find({
        savedBy: userId
      }).toArray();
